      ...
      (async function getProduct() {
        try {
          const { data } = await axios.get(
            `${BASE_URL}/googleSchema/practice/${id}`
          );
          let schema;
          if (data.message === 'Google schema practice set data') {
            schema = {
              '@context': 'https://schema.org/',
              '@type': 'Product',
              name: data.data[0].setName,
              image: [''],
...
export default usePracticeSchema;
