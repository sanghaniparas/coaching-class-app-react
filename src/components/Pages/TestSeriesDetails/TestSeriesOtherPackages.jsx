...
const TestSeriesOtherPackages = ({ testDetails, match }) => {
  ...
  useEffect(() => {
    async function fetchDifferentTestPackage() {
      try {
        const response = await axios.get(
          `${BASE_URL}/testPackage/similarTestPackages/${match.params.id}`
        );
        console.log("other test", response.data.data);
  ...
  useEffect(() => {
    ...
    if (
      ...
      res?.length && res.map((el) => checkInWishList(el.id));
    }
  }, [res]);
  ...
  return (
    <Carousel heading="">
      <h1 style={{ fontWeight: "900", fontSize: "22px", marginBottom: "1rem" }}>
        {testDetails 
          ? `Other Test Packages "${testDetails?.coachingDetails?.coachingName}"`
          : null}
      </h1>
      {res?.length !== 0 &&
        <ReactCarousel
                      ...
                      <div className="a-listTop">
                        ...
                        >
                          ...
                          <div className="a-ratingandstars">
                            ...
                            <b>
                              (
                          {item.ratingCount && item.ratingCount.toFixed(1) > 2
                        ...
                        <p className="a-location">
                          ...
                          {item.coaching.coachingName}{" , "} {item.coaching.city.city} <br />
                          {item.coaching.state.name}
                        </p>
                        <p
...
export default withRouter(TestSeriesOtherPackages);
