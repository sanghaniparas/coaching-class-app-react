...
export default function MostAttempted({ mostAttempedList, pageDetails }) {
  ...
  useEffect(() => {
    if (
      ...
      mostAttempedList.map((el) => checkInWishList(el.id));
    }
  }, []);
                    ...
                    <div className="a-listTop">
                      ...
                      {item &&
                        item.examTypes.map((i, idx) => (
                          <p className="a-typeExam" key={idx}>
                            {i.examType}
                          </p>
                        ))}
                      <ul
